...
def ground_truth(labfile):
	...
	'c':12,'c#':13,'d':14,'d#':15,'e':16,'f':17,'f#':18,'g':19,'g#':20,'a':21,'a#':22,'b':23,'A:min':24,'A:min/b7':25,'F:maj7':26,'F:maj6':27}
	...
	with open(labfile,'r') as g:
          ...
          for line in f:
              ...
              #print('a:',a)
              list.append(a)
              ground[int(np.floor(float(a[0])*86.13)):int(np.floor(float(a[1])*86.13))] = dict[a[2].rstrip("\n")]
             # print(ground)
              if float(a[1])>15:
                  break
...
ground = ground_truth('./sample_data/ground_truth_samples/Let It Be.lab')
